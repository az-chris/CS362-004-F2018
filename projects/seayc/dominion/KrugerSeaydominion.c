...
void adventurerCard(struct gameState *state, int handPos, int currentPlayer)
{
	//removed next player advancement logic from this function
    int temphand[MAX_HAND];
	int drawntreasure = 0;
	int cardDrawn;
    int leave = 0;  //fixes infinite loop
	int z = 0;
	while (drawntreasure < 2) //fixes extra treasure
    { 
		if (state->deckCount[currentPlayer] < 1) {
            if(leave == 0)  //fixes infinite loop
            {
...
void councilRoomCard(struct gameState* state, int handPos, int currentPlayer)
{
     for (int i = 0; i < 4; i++)
     {
          drawCard(currentPlayer, state);
